...
void destruct_tree(obj obj){
  tree_t *tree = (tree_t *)obj;
  release(tree->top);
  //allt annat är funktioner och en int så skiter i
  return;
}
void destruct_node(obj obj){
  node_t *node = (node_t *)obj;
  release(node->left);
  release(node->right);
  return;
}
void destruct_elem(obj obj){
  //HUR SER ELEM UT
  return;
}
void destruct_key(obj obj){
  destruct_elem(obj);
}
...
{
  tree_t *new = allocate(sizeof(tree_t), destruct_tree);
...
bool tree_insert(tree_t *tree, tree_key_t key, elem_t value)
{
  ...
  if (*leaf == NULL)
    {
      node_t *new = allocate(sizeof(node_t), destruct_node);
      new->key = key;
      new->value = value;
...
elem_t *tree_elements(tree_t *tree)
{
  ...
  elem_t *values = allocate(sizeof(elem_t), destruct_elem);
...
tree_key_t *tree_keys(tree_t *tree)
{
  assert(tree);
  int size = tree_size(tree);
  tree_key_t *keys = allocate(sizeof(tree_key_t), destruct_key);
