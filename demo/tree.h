...
/// \param compare (may be NULL) used to compare keys. If compare is NULL, keys will be assumed to be integers.
/// \returns: empty tree
tree_t *tree_new(element_copy_fun element_copy, key_free_fun key_free, element_free_fun elem_free, element_comp_fun compare);

/// Remove a tree along with all obj elements.
///
/// \param tree the tree
...
bool tree_insert(tree_t *tree, tree_key_t key, obj elems);
...
/// \param result a pointer to where result can be stored (only defined when result is true). May not be null.
/// \returns: true if key is a key in the tree
...
/// \param tree pointer to the tree
/// \param key the key of elem to be removed
/// \param result a pointer to where result can be stored (only defined when result is true). May be null.
/// \returns: true if key is a key in the tree
...
/// \param result a obj where result will be stored (only defined when result is true)
...
typedef bool(*key_elem_apply_fun)(tree_key_t key, obj elem, void *data);
...
/// Checks if the tree is balanced. An empty tree is considered to be balanced.
///
/// \param tree pointer to the tree
/// \param depth the deepest difference in depth
/// \returns: true if tree is already balanced, otherwise false
bool tree_is_balanced(tree_t *tree, int *depth);

/// Balances the tree.
///
/// \param tree pointer to the tree
/// \returns: false if the tree is already balanced, otherwise true
