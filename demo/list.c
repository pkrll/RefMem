...
void destruct_link(obj obj){
  link_t *link = (link_t *)obj;
  //VAD GÖR MAN MED ELEM
  release(link->next);
}
void destruct_list(obj obj){
...
{
  list_t *new = allocate(sizeof(list_t), destruct_list);
  if (new)
    {
      new->copy_f = copy_fun;
...
{
  link_t *new = allocate(sizeof(link_t), destruct_link);
